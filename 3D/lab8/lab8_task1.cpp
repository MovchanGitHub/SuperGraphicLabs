...
unsigned char screen[SCREENW * SCREENH];
double z_buffer[SCREENW * SCREENH];

void set_pixel(const point& p, const ImColor& c) {
	if (p.x < 0 || p.x >= SCREENW || p.y < 0 || p.y >= SCREENH) return;
	if (p.z > z_buffer[static_cast<int>(p.x) * SCREENW + static_cast<int>(p.y)]) {
		z_buffer[static_cast<int>(p.x) * SCREENW + static_cast<int>(p.y)] = p.z;
		screen[static_cast<int>(p.x) * SCREENW + static_cast<int>(p.y)] = c.Value.x * 255;
	}
}

void set_screen(int val) {
	std::fill(screen, screen + SCREENW * SCREENH, val);
}
...
void DrawScreen() {
	for (int w = 0; w < SCREENW; ++w)
		for (int h = 0; h < SCREENH; ++h) {
			ImColor color{ screen[w * SCREENW + h], screen[w * SCREENW + h], screen[w * SCREENW + h], 255 };
			//if (pixels[(h * WIDTHSZ + w) * 4] != screen[w][h])
				//SetPixel(w, h, color);
			ImGui::GetForegroundDrawList()->
				AddRectFilled(ImVec2(w + 600, h + 200), ImVec2(w + 1 + 600, h + 1 + 200), color);
		}
}
...
point InterpolateVertexX(float x, const point& p1, const point& p2) {
	float t = (x - p1.x) / (p2.x - p1.x);
	return { x, p1.y, p1.z + t * (p2.z - p1.z) };
}
...
void DrawTriangle(const point& p1, const point& p2, const point& p3, const int color) {
	...
	for (float y = top.y; y < mid.y - 0.01; ++y) {
		...
		//for (int x = left.x; x < right.x - 0.01; ++x) {
		//    point curr = InterpolateVertexX(x, left, right);
		//    set_pixel(curr, { color, color, color, 255 });
		//}
...
class polyhedron {
	...
	point cross_product(const point& v1, const point& v2) {
